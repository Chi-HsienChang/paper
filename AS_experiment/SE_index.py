...
se_df = pd.read_csv("./data/SE_events_high.csv")
...
for i, row in se_df.iterrows():
    ...
    for line in text.splitlines():
        ...
        if re.match(r"^\d+,\s*\d+,\s*[0-9.]+$", line):
            fields = line.split(",")
            three, five, p = int(fields[0]), int(fields[1]), float(fields[2])
...
allparse_folder = "t_allParse_exon"
...
# 輸出結果
print("✅ 在 t_allParse_exon 中存在的 index：")
print(present)
print("len = ", len(present))

print("\n❌ 在 t_allParse_exon 中不存在的 index：")
print(missing)
