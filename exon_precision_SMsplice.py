    ...
    for top_k in tqdm(top_ks, desc="top_k", leave=False):
        for code, name in species_map.items():
            ...
            for txt_file in file_list:
                ...
                # 5. 檢查 SMsplice 預測的 exon pair，並計算分數
            ...
            if total_pred == 0:
                ...
                # 先算 overall_precision（所有含score=0者）
                overall_precision = len(correct_exon_scores) / total_pred

                # 為了算 precision_09 與 subset_09，要先排除 score=0
                correct_nonzero = [s for s in correct_exon_scores if s != 0]
                incorrect_nonzero = [s for s in incorrect_exon_scores if s != 0]
                total_pred_nonzero = len(correct_nonzero) + len(incorrect_nonzero)

                if total_pred_nonzero == 0:
                    # 都是零分預測 -> precision_09 沒意義
                    precision_09 = None
                    subset_09 = 0.0
                else:
                    # 看非零預測裡，分數≥0.9者
                    preds_above_09 = [s for s in (correct_nonzero + incorrect_nonzero) if s >= 0.9]
                    correct_above_09 = [s for s in correct_nonzero if s >= 0.9]

                    # precision_09 = 在非零且≥0.9 的預測中，有多少正確
                    if len(preds_above_09) > 0:
                        precision_09 = len(correct_above_09) / len(preds_above_09)
                        # subset_09 = (非零且≥0.9的預測數量) / (非零總預測數量)
                        subset_09 = len(preds_above_09) / total_pred_nonzero
                    else:
                        precision_09 = None
                        subset_09 = 0.0
...
print("Saved precision_09, subset_09, and overall_precision to exon_scores_top_k_class.csv")
