...
for seed in tqdm(seeds, desc="Seeds"):
    for top_k in tqdm(top_ks, desc="top_k", leave=False):
        for code, name in species_map.items():
            ...
            incorrect_a_correct = []  # 3' 邊界正確、5' 錯誤
            incorrect_b_correct = []  # 3' 邊界錯誤、5' 正確
            incorrect_both = []       # 3' 及 5' 邊界都錯
            ...
            for txt_file in file_list:
                ...
                # ============= 2) 建立 annotated / SMsplice 的 exon pair =============
                # 這裡的設計：exon pair 為 (3SS, 5SS)
                ann_pairs_list = (
                    [(0, ann5ss[0])] +
                ...
                # ============= 4) 若有表格 "3SS,5SS,prob" 也將其讀入，以該表分數為主 =============
                exon_table = {}
                ...
                # ============= 5) 對於 SMsplice 所預測的 exon pair，計算分數並判斷正確性 =============
                for (three_site, five_site) in sm_pairs_set:
                    ...
                    # ---- 以「整個 pair」是否在 annotated pairs 中判斷 correctness ----
                    ...
                    if is_correct:
                        ...
                        # 若要進一步區分「部分正確」情形
                        # 先檢查各自的 3' / 5' 邊界是否符合 annotated 
                        # （0 與 -1 視作正確邊界）
                        a_correct_flag = (three_site == 0) or (three_site in ann3ss)
                        b_correct_flag = (five_site == -1) or (five_site in ann5ss)
